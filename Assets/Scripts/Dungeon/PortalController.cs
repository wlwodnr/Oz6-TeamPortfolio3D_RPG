...
public class PortalController : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        bool wasAccepted = ControllerDungeonFlow.RequestMoveToFloor(FloorTarget);
    ...
    private void TestRequestFloorTransition()
    {
        ...
        {
            Debug.LogWarning(
                $"PortalController: [{gameObject.name}] " +
                "이동 설정이 완료되지 않았습니다.",
                this);
        ...
        bool wasAccepted =
            ControllerDungeonFlow.RequestMoveToFloor(
                FloorTarget);

        Debug.Log(
            $"PortalController: [{gameObject.name}] " +
            $"테스트 이동 요청 결과: {wasAccepted}",
            this);
    }
#endif
}
